...
# index.html is deliberately generated and gitignored. Preserve any local copy, then prove two builds are identical.
index = R / 'index.html'
...
try:
    ...
    proc = subprocess.Popen(
        [sys.executable, 'tools/serve.py', '--no-browser', '--port', str(port)],
    ...
    try:
        ...
        assert response.read() == first and response.headers['X-Content-Type-Options'] == 'nosniff'
        ok('可选标准库HTTP服务真实返回完整发行HTML及nosniff头')
        try:
            urllib.request.urlopen(f'http://127.0.0.1:{port}/docs/', timeout=1)
        except urllib.error.HTTPError as e:
            assert e.code == 403
        else:
            raise AssertionError('directory listing not denied')
        ok('HTTP服务不开放目录列表')
    finally:
        proc.terminate()
        try:
